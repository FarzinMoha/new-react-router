...
export default function SignUp() {
  ...
  const onSubmitHandler = async (e:any) => {
    ...
    checkRequired()
    if(errors) return
    await submit({ email: state.email.value, password: state.password.value },{ method: "post" });
  }

  return (
    <main className="w-full min-h-screen bg-amber-500 flex justify-center items-center">
      <Form onSubmit={onSubmitHandler} method="post" className="w-full max-w-[500px] flex flex-col justify-start bg-amber-600 my-1 mx-2 p-3">
          ...
          <Btn type="submit" primary  disabled={!!errors || isLoading} isLoading={isLoading}>Accept</Btn>
